...
Mnemonic MnemonicsTab[] =
{
  ...
  {"Increment L", "INC L", 1},
  {"Decrement L", "DEC L", 1},
  {"Load 8-bit immediate into L", "LD L,0x%X", 2},
  {"Complement (logical NOT) on A", "CPL", 1},
  {"Relative jump by signed immediate if last result caused no carry", "JR NC,%i", 2},
  {"Load 16-bit immediate into SP", "LD SP,0x%X", 3},
  ...
  {"Load 8-bit immediate into address pointed by HL", "LD (HL),0x%X", 2},
  {"Set carry flag", "SCF", 1},
  {"Relative jump by signed immediate if last result caused carry", "JR C,%i", 2},
  ...
  {"Increment A", "INC A", 1},
  {"Decrement A", "DEC A", 1},
  {"Load 8-bit immediate into A", "LD A,0x%X", 2},
  ...
  {"Absolute jump to 16-bit location if last result was not zero", "JP NZ,0x%X", 3},
  {"Absolute jump to 16-bit location", "JP 0x%X", 3},
  {"Call routine at 16-bit location if last result was not zero", "CALL NZ,0x%X", 3},
  {"Push 16-bit BC onto stack", "PUSH BC", 1},
  {"Add 8-bit immediate to A", "ADD A,0x%X", 2},
  {"Call routine at address 0000h", "RST 0", 1},
  {"Return if last result was zero", "RET Z", 1},
  {"Return to calling routine", "RET", 1},
  {"Absolute jump to 16-bit location if last result was zero", "JP Z,0x%X", 3},
  {"Extended operations (two-byte instruction code)", "Ext ops", 1},
  {"Call routine at 16-bit location if last result was zero", "CALL Z,0x%X", 3},
  {"Call routine at 16-bit location", "CALL 0x%X", 3},
  {"Add 8-bit immediate and carry to A", "ADC A,0x%X", 2},
  ...
  {"Absolute jump to 16-bit location if last result caused no carry", "JP NC,0x%X", 3},
  {"Operation removed in this CPU", "XX", 1},
  {"Call routine at 16-bit location if last result caused no carry", "CALL NC,0x%X", 3},
  {"Push 16-bit DE onto stack", "PUSH DE", 1},
  {"Subtract 8-bit immediate from A", "SUB A,0x%X", 2},
  ...
  {"Absolute jump to 16-bit location if last result caused carry", "JP C,0x%X", 3},
  {"Operation removed in this CPU", "XX", 1},
  {"Call routine at 16-bit location if last result caused carry", "CALL C,0x%X", 3},
  {"Operation removed in this CPU", "XX", 1},
  {"Subtract 8-bit immediate and carry from A", "SBC A,0x%X", 2},
  {"Call routine at address 0018h", "RST 18", 1},
  {"Save A at address pointed to by (FF00h + 8-bit immediate)", "LDH (0x%X),A", 2},
  ...
  {"Logical AND 8-bit immediate against A", "AND 0x%X", 2},
  {"Call routine at address 0020h", "RST 20", 1},
  {"Add signed 8-bit immediate to SP", "ADD SP,0x%X", 2},
  {"Jump to 16-bit value pointed by HL", "JP (HL)", 1},
  {"Save A at given 16-bit address", "LD 0x%X,A", 3},
  {"Operation removed in this CPU", "XX", 1},
  {"Operation removed in this CPU", "XX", 1},
  {"Operation removed in this CPU", "XX", 1},
  {"Logical XOR 8-bit immediate against A", "XOR 0x%X", 2},
  {"Call routine at address 0028h", "RST 28", 1},
  {"Load A from address pointed to by (FF00h + 8-bit immediate)", "LDH A,(0x%X)", 2},
  {"Pop 16-bit value from stack into AF", "POP AF", 1},
  {"Operation removed in this CPU", "XX", 1},
  {"DIsable interrupts", "DI", 1},
  {"Operation removed in this CPU", "XX", 1},
  {"Push 16-bit AF onto stack", "PUSH AF", 1},
  {"Logical OR 8-bit immediate against A", "OR 0x%X", 2},
  {"Call routine at address 0030h", "RST 30", 1},
  {"Add signed 8-bit immediate to SP and save result in HL", "LDHL SP,0x%X", 2},
  {"Copy HL to SP", "LD SP,HL", 1},
  {"Load A from given 16-bit address", "LD A,0x%X", 3},
  {"Enable interrupts", "EI", 1},
  {"Operation removed in this CPU", "XX", 1},
  {"Operation removed in this CPU", "XX", 1},
  {"Compare 8-bit immediate against A", "CP 0x%X", 2},
...
int Debugger::GetOpNbArgs(Emulator *emu, int addr)
{
  BYTE	value;

  value = emu->ReadMem(addr);
  if (value != 0xCB)
    return MnemonicsTab[value].size - 1;
  else
    return 0;
  return 0;
}
