  ...
  for (int i = 0; i < 160; i++)
    {
      ...
      SetColor((int *)(curScanLine + i * 4), bit0, bit1);
      posX++;
    }
  mGraphics->DrawScanLine(curLine);
  //bit1 = //IS_BIT_SET(x, (((drawline + scrollY) % 8)) + 0);
  //bit2 = IS_BIT_SET(x, (((drawline + scrollY) % 8)) + 1);

}

void	Emulator::SetColor(int *scanLine, bool bit0, bool bit1)
{
  int	spriteColor;
  int	finalColor;
